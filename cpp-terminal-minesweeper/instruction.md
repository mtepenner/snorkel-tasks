...
for the api contract, use GET http://localhost:8080/board which returns json with status, mines, and board_state, and POST http://localhost:8080/move with json body {"x": integer, "y": integer, "action": "reveal"} which returns {"status":"cleared"}. client binary needs to be compiled to /app/workspace/client.
