...
def trigger_etl():
    try:
        ...
        for record in all_data:
            c.execute("INSERT INTO records (data) VALUES (?)", (json.dumps(record),))
        conn.commit()
        conn.close()
        
        log_event("ETL processed successfully.")
        return jsonify({"status": "success", "message": "ETL processed successfully."})
