...
def test_m1_preprocessing_and_retrieval(client):
    ...
    # STRENGTHENED: Verify imputation
    vals = [row["A"] for row in data]
    assert None not in vals, "Missing values not handled"
    
    # Check for mean imputation OR scaling logic applying to the imputed value
    # (Since scaling is required, the mean value 20 will be 0.0 after standard scaling)
    assert pytest.approx(0.0) in vals or pytest.approx(20.0) in vals, "Imputation or scaling logic failed"

    # STRENGTHENED: Upload categorical data and verify one-hot encoding
