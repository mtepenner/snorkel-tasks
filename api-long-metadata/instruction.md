...
* you have to parse the text chunks themselves to figure out the `topics`. Extract between 3 and 10 meaningful non-empty keyword strings that represent the document's main subjects. Use word-frequency analysis or another reasonable content-based technique, and filter out obvious stop-words.
...
* I need a quick way to test this manually, so throw together a really ugly test UI at `/app/workspace/src/static/index.html`. Your FastAPI setup must mount/serve that directory at `/static` using `StaticFiles`, so the page is reachable at `/static/index.html` and I don't get 404s.
