...
* Includes an `analysis` object with these exact computed fields: `mean_velocity`, `velocity_stddev`, `pressure_drop`, and `dominant_regime` (must evaluate to exactly `"laminar"`, `"transitional"`, or `"turbulent"`).
